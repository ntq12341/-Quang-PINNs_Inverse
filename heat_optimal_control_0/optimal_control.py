...
def make_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="PINN optimal control solver for heat example 5.1.2 with exact minimum-energy control.")
    ...
    parser.add_argument("--outdir", type=str, default="pinns_optimal_control_ill-posed/outputs/heat_optimal_control_0")
    return parser
...
def _train_once(args: argparse.Namespace, alpha: float, epochs: int, print_every: int) -> tuple[NormalizedMLP, NormalizedMLP, OptimalControlHistory]:
    ...
    for epoch in range(1, epochs + 1):
        ...
        if epoch % print_every == 0 or epoch == 1 or epoch == epochs:
            print(
                f"[heat control 0] epoch={epoch:5d} total={total:.3e} "
...
def l_curve_method(args: argparse.Namespace) -> tuple[float, list[dict[str, float]]]:
    ...
    for alpha in alpha_values:
        ...
        print(f"[heat alpha 0] alpha={alpha:.3e} residual={residual:.3e} reg={reg:.3e}")
...
def train(args: argparse.Namespace) -> tuple[NormalizedMLP, NormalizedMLP, OptimalControlHistory, float]:
    if args.alpha_method == "lcurve":
        ...
        print(f"[heat control 0] selected alpha by L-curve: {alpha:.3e}")
    else:
...
def evaluate_and_save(u_net: NormalizedMLP, f_net: NormalizedMLP, history: OptimalControlHistory, alpha: float, args: argparse.Namespace) -> None:
    ...
    print(f"[heat control 0] exact coefficients: A={COEFF_A:.4f}, B={COEFF_B:.4f}, C={COEFF_C:.4f}")
    print(f"[heat control 0] relative L2 error for u*: {rel_u:.3e}")
    print(f"[heat control 0] relative L2 error for f*: {rel_f:.3e}")
    print(f"[heat control 0] terminal objective J_roll: {objective_roll:.3e}")
    print(f"[heat control 0] H1 regularization term: {h1_roll:.3e}")
    print(f"[heat control 0] Tikhonov functional: {tikhonov_roll:.3e}")
