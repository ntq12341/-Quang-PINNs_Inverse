...
def make_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Sweep wJ for heat optimal control example 5.1.2.")
    ...
    parser.add_argument("--outdir", type=str, default="pinns_optimal_control_ill-posed/outputs/heat_optimal_control_0/sweep_wj")
    return parser
...
def main() -> None:
    ...
    for wj in wj_values:
        ...
        run_args = argparse.Namespace(
            ...
            epochs=args.epochs,
            lr=1e-3,
            lr_drop_epochs=[max(1, int(args.epochs / 2))],
            lr_drop_factor=0.1,
            ...
            alpha_scan_epochs=args.alpha_scan_epochs,
        ...
        rows.append((
            wj,
            alpha,
            float(loss_hist[-1, 2] + loss_hist[-1, 3] + loss_hist[-1, 4]),
            float(loss_hist[-1, 5]),
            float(loss_hist[-1, 6]),
            float(summary[-1, 2]),
            float(summary[-1, 3]),
        ...
        print(
            f"[heat sweep 0] wJ={wj:.3e} alpha={alpha:.3e} "
            f"LFBI={rows[-1][2]:.3e} LJ={rows[-1][3]:.3e} Reg={rows[-1][4]:.3e} "
            f"rel_u={rows[-1][5]:.3e} rel_f={rows[-1][6]:.3e} "
            f"Jroll={rows[-1][7]:.3e} Tik={rows[-1][8]:.3e}"
        )
    ...
    save_named_columns_csv(
        result_dir / "summary.csv",
        {
            ...
            "rel_u": arr[:, 5],
            "rel_f": arr[:, 6],
            "objective_rollout": arr[:, 7],
            "tikhonov_rollout": arr[:, 8],
        },
    )
    ...
    best = int(np.flatnonzero(valid_mask)[np.argmin(arr[valid_mask, 7])])
    ...
    print(f"[heat sweep 0] copied best run to: {canonical_dir}")
    print(f"[heat sweep 0] best wJ by rollout objective: {arr[best, 0]:.3e}")
