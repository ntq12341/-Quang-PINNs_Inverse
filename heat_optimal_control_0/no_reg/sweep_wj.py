...
def make_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Sweep wJ for heat optimal control example 5.1.2 without regularization.")
    ...
    parser.add_argument("--outdir", type=str, default="pinns_optimal_control_ill-posed/outputs/heat_optimal_control_0/no_reg/sweep_wj")
    return parser
...
def main() -> None:
    ...
    for wj in args.wJ_list:
        ...
        rows.append((wj, float(loss_hist[-1, 2] + loss_hist[-1, 3] + loss_hist[-1, 4]), float(loss_hist[-1, 5]), float(summary[-1, 1]), float(summary[-1, 2]), float(summary[-1, 4])))
        paths.append(str(run_dir / "optimal_control_results"))
        print(
            f"[heat no_reg sweep 0] wJ={wj:.3e} "
            f"LFBI={rows[-1][1]:.3e} LJ={rows[-1][2]:.3e} "
            f"rel_u={rows[-1][3]:.3e} rel_f={rows[-1][4]:.3e} Jroll={rows[-1][5]:.3e}"
        )

    result_dir = outdir / "sweep_wj_results"
    result_dir.mkdir(parents=True, exist_ok=True)
    arr = np.array(rows, dtype=np.float64)
    control_mod.save_named_columns_csv(
    ...
    print(f"[heat no_reg sweep 0] best wJ by rollout objective: {arr[best, 0]:.3e}")
