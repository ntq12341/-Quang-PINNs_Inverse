...
class PINN(nn.Module):
    """PINN for 3D Poisson Cauchy problem (Example 5.5)."""

    def __init__(self, layers=None):
        super().__init__()
        if layers is None:
            # Paper setup: 4 hidden layers, 10 neurons each.
            layers = [3, 10, 10, 10, 10, 1]
    ...
    def forward(self, x1, x2, x3):
        ...
        x3n = 2.0 * (x3 - X3_MIN) / (X3_MAX - X3_MIN) - 1.0

        u = torch.cat([x1n, x2n, x3n], dim=1)
...
def _add_dirichlet_loss(model, data, key, losses, total_loss, w, device):
    ...
    x1, x2, x3, u_ref = data[key]
    u_pred = model(_to_tensor(x1, device), _to_tensor(x2, device), _to_tensor(x3, device))
...
def compute_loss(model, data, alpha=0.0, device="cpu", loss_weights=None):
    ...
    x1_r, x2_r, x3_r, h_r = data["residual"]
    x1_r_t = _to_tensor(x1_r, device, requires_grad=True)
    x2_r_t = _to_tensor(x2_r, device, requires_grad=True)
    x3_r_t = _to_tensor(x3_r, device, requires_grad=True)
    h_r_t = _to_tensor(h_r, device)

    u_r = model(x1_r_t, x2_r_t, x3_r_t)
    u_x1 = torch.autograd.grad(u_r, x1_r_t, grad_outputs=torch.ones_like(u_r), create_graph=True)[0]
    u_x2 = torch.autograd.grad(u_r, x2_r_t, grad_outputs=torch.ones_like(u_r), create_graph=True)[0]
    u_x3 = torch.autograd.grad(u_r, x3_r_t, grad_outputs=torch.ones_like(u_r), create_graph=True)[0]

    u_x1x1 = torch.autograd.grad(u_x1, x1_r_t, grad_outputs=torch.ones_like(u_x1), create_graph=True)[0]
    u_x2x2 = torch.autograd.grad(u_x2, x2_r_t, grad_outputs=torch.ones_like(u_x2), create_graph=True)[0]
    u_x3x3 = torch.autograd.grad(u_x3, x3_r_t, grad_outputs=torch.ones_like(u_x3), create_graph=True)[0]

    pde_res = u_x1x1 + u_x2x2 + u_x3x3 - h_r_t
    losses["pde"] = torch.mean(pde_res**2)
    total_loss = total_loss + w("pde") * losses["pde"]

    x1_c, x2_c, x3_c, u_c, g_c = data["cauchy"]
    x1_c_t = _to_tensor(x1_c, device, requires_grad=True)
    x2_c_t = _to_tensor(x2_c, device, requires_grad=True)
    x3_c_t = _to_tensor(x3_c, device, requires_grad=True)
    u_c_t = _to_tensor(u_c, device)
    g_c_t = _to_tensor(g_c, device)

    u_c_pred = model(x1_c_t, x2_c_t, x3_c_t)
    u_c_x3 = torch.autograd.grad(
        u_c_pred,
        x3_c_t,
    ...
    # On G1 (x3=0), outward normal is n=(0,0,-1) => ?u/?n = -u_x3.
    g_c_pred = -u_c_x3
    ...
    total_loss = _add_dirichlet_loss(model, data, "gamma5", losses, total_loss, w, device)
    total_loss = _add_dirichlet_loss(model, data, "gamma6", losses, total_loss, w, device)

    x1_k, x2_k, x3_k = data["regularization"]
    x1_k_t = _to_tensor(x1_k, device)
    x2_k_t = _to_tensor(x2_k, device)
    x3_k_t = _to_tensor(x3_k, device, requires_grad=True)

    u_k = model(x1_k_t, x2_k_t, x3_k_t)
    u_k_n = torch.autograd.grad(u_k, x3_k_t, grad_outputs=torch.ones_like(u_k), create_graph=True)[0]
...
def discrepancy_principle_method(
    ...
    results = []
    target_rms = max(float(tau) * float(noise_level) * np.sqrt(max(len(discrepancy_terms), 1)), 1e-12)
