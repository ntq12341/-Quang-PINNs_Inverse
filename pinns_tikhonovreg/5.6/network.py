...
def _add_dirichlet_loss(model, data, key, losses, total_loss, w, device):
    ...
    scale = torch.sqrt(torch.mean(u_obs**2) + 1e-12)
    losses[key] = torch.mean(((u_pred - u_obs) / scale) ** 2)
...
def compute_loss(model, data, alpha=0.0, device="cpu", loss_weights=None):
    """Compute total Tikhonov-regularized loss for Example 5.6.

    PDE term: -div(q(u)*grad(u)) = h
      => L[u] = -(q'(u)*|grad u|^2 + q(u)*Delta u) - h  = 0
      where q(u)=1+u^2, q'(u)=2u.

    Regularization on Gamma2 (x2=0.5): ||\partial u/\partial n||^2_{L2(Gamma2)}.
    Outward normal on Gamma2 is n=(0,+1), so du/dn = du/dx2.
    """
    ...
    # Normalize by RHS scale to prevent PDE residual from dominating all terms.
    pde_scale = torch.sqrt(torch.mean(h_r_t**2) + 1e-12)
    losses["pde"] = torch.mean((pde_res / pde_scale) ** 2)
    ...
    u_scale = torch.sqrt(torch.mean(u_c_t**2) + 1e-12)
    g_scale = torch.sqrt(torch.mean(g_c_t**2) + 1e-12)
    losses["cauchy_u"] = torch.mean(((u_c_pred - u_c_t) / u_scale) ** 2)
    losses["cauchy_g"] = torch.mean(((g_c_pred - g_c_t) / g_scale) ** 2)
    ...
    # -----------------------------------------------------------------------
    # 4. Tikhonov regularization: ||du/dn||^2 on Gamma2 (x2=0.5), n=(0,+1)
    # -----------------------------------------------------------------------
    x1_k, x2_k = data["regularization"]
    x1_k_t = _to_tensor(x1_k, device)
    x2_k_t = _to_tensor(x2_k, device, requires_grad=True)

    u_k = model(x1_k_t, x2_k_t)
    u_k_n = torch.autograd.grad(
        u_k, x2_k_t, grad_outputs=torch.ones_like(u_k), create_graph=True
    )[0]
