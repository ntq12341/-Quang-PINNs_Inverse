...
def summarize_suite_errors(all_results):
    """
    Convert output of run_example_51_suite() into a flat table.

    Expected entry keys:
    - noise_level
    - alpha  (có thể là None nếu không dùng regularization)
    - u_top_exact
    - u_top_pred
    """
    rows = []
    for noise_level, result in sorted(all_results.items()):
        ...
        # FIX: alpha có thể là None khi không dùng regularization → dùng np.nan
        raw_alpha = result.get("alpha", np.nan)
        alpha_val = float(raw_alpha) if raw_alpha is not None else np.nan
...
def export_error_tables(all_results, output_dir="results/tables"):
    """
    Export CSV files:
    - summary-noise.csv  : noise, alpha, L1, Linf, L2, RMSE, SDE, CV_percent
    - <error_type>-noise.csv : noise, error  (một file cho mỗi loại lỗi)
    """
