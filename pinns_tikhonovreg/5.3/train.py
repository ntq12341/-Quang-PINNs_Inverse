...
from data_generate import (
    X1_MAX,
    X2_MAX,
    exact_solution_example_53,
    generate_forward_data,
    generate_inverse_data,
    top_boundary_example_53_step,
)
...
def _predict_field(model, device, exact_field_fn=None, nx=140, ny=80):
...
def run_example_53(
    ...
    """Run Example 5.3 (step top boundary) from Hao-Shishlenin-Cong (2025)."""
    if device is None:
        device = "cuda" if torch.cuda.is_available() else "cpu"

    print(f"\n=== Example 5.3 | noise={noise_level * 100:.1f}% | device={device} ===\n")

    layers = [2, 7, 7, 7, 7, 1]  # 4 hidden layers, 7 neurons each
    ...
    model_forward = PINN(layers=layers).to(device)
    data_forward = generate_forward_data(
        N_r=10000,
        N_b=2000,
        top_boundary_fn=top_boundary_example_53_step,
    )
    train_lbfgs(
        model_forward,
        data_forward,
    ...
    x_bottom = np.linspace(0.0, X1_MAX, 600)
    ...
    model_for_alpha = PINN(layers=layers).to(device)
    ...
    model_final = PINN(layers=layers).to(device)
    losses = train_lbfgs(
        model_final,
        data_inverse,
        alpha=optimal_alpha,
    ...
    x_eval = np.linspace(0.0, X1_MAX, 600)
    ...
    u_top_exact = top_boundary_example_53_step(x_eval)
    ...
    x_field, y_field, u_field_exact, u_field_pred = _predict_field(
        model_final,
        device,
        exact_field_fn=exact_solution_example_53,
    )
...
def run_example_53_suite(
    ...
    """Run Example 5.3 for multiple noise levels, save figures, and export tables."""
    if device is None:
        device = "cuda" if torch.cuda.is_available() else "cpu"
    ...
    for noise in noise_levels:
        ...
        plot_surface_comparison(
            ...
            save_path=figs_path / f"example53-surface-noise-{noise * 100:g}.png",
        )
    ...
    plot_top_boundary_comparison(
        ...
        u_top_exact,
        preds_by_noise,
        save_path=figs_path / "example53-top-boundary-all-noise.png",
    )
