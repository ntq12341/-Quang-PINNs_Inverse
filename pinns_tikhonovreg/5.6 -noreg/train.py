...
def run_example_56(
    ...
    No regularization term in this variant (alpha is fixed to 0).
    """
    np.random.seed(seed)
    ...
    if alpha_method.lower() == "fixed":
        optimal_alpha = 0.0 if abs(float(fixed_alpha)) > 0 else float(fixed_alpha)
        alpha_scan = []
        discrepancy_target = None
        print(f"Using fixed alpha: {optimal_alpha:.6e}")

    elif alpha_method.lower() in {"lcurve", "discrepancy"}:
        # This folder is explicitly "noreg": alpha should not be tuned.
        optimal_alpha = 0.0
        alpha_scan = []
        discrepancy_target = None
        print(
            "This is the no-regularization variant; "
            "forcing alpha=0 and skipping alpha selection."
        )
    else:
