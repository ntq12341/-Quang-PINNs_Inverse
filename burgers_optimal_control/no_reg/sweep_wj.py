...
def make_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Sweep wJ for Burgers PINN optimal control without regularization.")
    ...
    parser.add_argument("--outdir", type=str, default="pinns_optimal_control_ill-posed/outputs/burgers_optimal_control/no_reg/sweep_wj")
    return parser
...
def main() -> None:
    ...
    for wj in args.wJ_list:
        ...
        print(f"[burgers no_reg sweep] wJ={wj:.3e} LFBI={rows[-1][1]:.3e} LJ={rows[-1][2]:.3e} Jroll={rows[-1][3]:.3e}")
    ...
    print(f"[burgers no_reg sweep] best wJ by rollout objective: {arr[best, 0]:.3e}")
