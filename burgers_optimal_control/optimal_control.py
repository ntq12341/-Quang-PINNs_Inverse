...
def make_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="PINN optimal control solver for the Burgers example.")
    ...
    parser.add_argument("--rollout-nt", type=int, default=400)
    parser.add_argument("--outdir", type=str, default="pinns_optimal_control_ill-posed/outputs/burgers_optimal_control")
    return parser
...
def _compute_batch_losses(
    ...
    u_r = u_net(xt_batch)
    f_r = f_net(xt_batch)
    pde_loss = torch.mean(burgers_residual(u_r, xt_batch, args.nu, f_r) ** 2)
    bc_loss = torch.mean(u_net(left_t) ** 2) + torch.mean(u_net(right_t) ** 2)
    ic_loss = torch.mean((u_net(init_t) - analytic_initial_condition(init_t[:, :1], nu=args.nu)) ** 2)
...
def l_curve_method(args: argparse.Namespace) -> tuple[float, list[dict[str, float]]]:
    ...
    for alpha in alpha_values:
        ...
        print(f"[burgers alpha] alpha={alpha:.3e} residual={residual:.3e} reg={reg:.3e}")
    ...
    # Avoid picking unstable endpoint curvatures on coarse L-curves.
    if len(score) >= 3:
        score[0] = -np.inf
        score[-1] = -np.inf
...
def train(args: argparse.Namespace) -> tuple[NormalizedMLP, NormalizedMLP, OptimalControlHistory, float]:
    if args.alpha_method == "lcurve":
        ...
        print(f"[burgers control] selected alpha by L-curve: {alpha:.3e}")
    else:
...
def evaluate_and_save(u_net: NormalizedMLP, f_net: NormalizedMLP, history: OptimalControlHistory, alpha: float, args: argparse.Namespace) -> None:
    ...
    x = torch.linspace(0.0, args.L, args.eval_nx + 2, device=device)[1:-1]
    ...
    with torch.no_grad():
        ...
        u_true = analytic_optimal_u(xx, tt, nu=args.nu)
        f_true = analytic_optimal_f(xx, tt, nu=args.nu)
    solver = BurgersForwardSolver(nx=args.rollout_nx, nt=args.rollout_nt, L=args.L, T=args.T, nu=args.nu)
    ...
    terminal_target_np = analytic_terminal_target(torch.tensor(solver.x, dtype=torch.float64), T=args.T, nu=args.nu).detach().cpu().numpy()
    ...
    x_term = torch.linspace(0.0, args.L, args.eval_nx + 2, device=device)[1:-1].reshape(-1, 1)
    xt_term = torch.cat([x_term, args.T * torch.ones_like(x_term)], dim=1)
    with torch.no_grad():
        u_term_pinn = u_net(xt_term).reshape(-1)
        target_term = analytic_terminal_target(x_term, T=args.T, nu=args.nu).reshape(-1)
    ...
    print(f"[burgers control] relative L2 error for u*: {rel_u:.3e}")
    print(f"[burgers control] relative L2 error for f*: {rel_f:.3e}")
    print(f"[burgers control] terminal objective J_roll: {objective_roll:.3e}")
    print(f"[burgers control] H1 regularization term: {h1_roll:.3e}")
    print(f"[burgers control] Tikhonov functional: {tikhonov_roll:.3e}")
