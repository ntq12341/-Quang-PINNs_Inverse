...
def analytic_burgers(x: torch.Tensor, t: torch.Tensor, nu: float = 0.01) -> torch.Tensor:
    expo = torch.exp(-(math.pi**2) * nu * (t - 5.0))
    num = 2.0 * nu * math.pi * expo * torch.sin(math.pi * x)
    den = 2.0 + expo * torch.cos(math.pi * x)
    return num / den


def analytic_initial_condition(x: torch.Tensor, nu: float = 0.01) -> torch.Tensor:
    return analytic_burgers(x, torch.zeros_like(x), nu=nu)


def analytic_terminal_target(x: torch.Tensor, T: float = 5.0, nu: float = 0.01) -> torch.Tensor:
    return analytic_burgers(x, torch.full_like(x, T), nu=nu)
