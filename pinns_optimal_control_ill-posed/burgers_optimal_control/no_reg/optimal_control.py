...
def make_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Burgers optimal control PINN without Tikhonov regularization.")
    ...
    parser.add_argument("--outdir", type=str, default="pinns_optimal_control_ill-posed/outputs/burgers_optimal_control/no_reg")
    return parser
...
def train(args: argparse.Namespace) -> tuple[NormalizedMLP, NormalizedMLP, NoRegHistory]:
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        if epoch % args.print_every == 0 or epoch == 1 or epoch == args.epochs:
            print(
                f"[burgers no_reg] epoch={epoch:5d} total={total:.3e} "
...
def evaluate_and_save(u_net: NormalizedMLP, f_net: NormalizedMLP, history: NoRegHistory, args: argparse.Namespace) -> None:
    ...
    print(f"[burgers no_reg] relative L2 error for u*: {rel_u:.3e}")
    print(f"[burgers no_reg] relative L2 error for f*: {rel_f:.3e}")
    print(f"[burgers no_reg] terminal objective J_roll: {objective_roll:.3e}")
