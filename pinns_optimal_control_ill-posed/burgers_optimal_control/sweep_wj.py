...
def make_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--outdir", type=str, default="pinns_optimal_control_ill-posed/outputs/burgers_optimal_control/sweep_wj")
    return parser
...
def main() -> None:
    ...
    for wj in wj_values:
        ...
        print(f"[burgers sweep] wJ={wj:.3e} alpha={alpha:.3e} LFBI={rows[-1][2]:.3e} LJ={rows[-1][3]:.3e} Reg={rows[-1][4]:.3e} Jroll={rows[-1][5]:.3e} Tik={rows[-1][6]:.3e}")
    ...
    print(f"[burgers sweep] copied best run to: {canonical_dir}")
    print(f"[burgers sweep] best wJ by rollout objective: {arr[best, 0]:.3e}")
