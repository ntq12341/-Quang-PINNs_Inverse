...
def make_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="PINN forward solver for the Burgers example on [0,1] x [0,1].")
    ...
    parser.add_argument("--outdir", type=str, default="pinns_optimal_control_ill-posed/outputs/burgers/forward")
    return parser
...
def train(args: argparse.Namespace) -> tuple[NormalizedMLP, TrainHistory, list[int], list[float]]:
    ...
    x_eval = torch.linspace(0.0, args.L, args.eval_nx + 2, device=device)[1:-1]
...
def evaluate_and_save(
    ...
    print(f"[burgers forward] grid L2 error: {l2_grid:.3e}")
